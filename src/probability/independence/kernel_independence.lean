...
def Indep_setsₖ [measurable_space Ω] (π : ι → set (set Ω)) (ν : α → measure Ω)
...
def indep_setsₖ [measurable_space Ω] (s1 s2 : set (set Ω)) (ν : α → measure Ω)
...
def Indepₖ (m : ι → measurable_space Ω) [measurable_space Ω] (ν : α → measure Ω)
  (μ : measure α . volume_tac) :
  Prop :=
Indep_setsₖ (λ x, {s | measurable_set[m x] s}) ν μ

/-- Two measurable space structures (or σ-algebras) `m₁, m₂` are independent with respect to a
measure `μ` (defined on a third σ-algebra) if for any sets `t₁ ∈ m₁, t₂ ∈ m₂`,
`μ (t₁ ∩ t₂) = μ (t₁) * μ (t₂)` -/
def indepₖ (m₁ m₂ : measurable_space Ω) [measurable_space Ω] (ν : α → measure Ω)
  (μ : measure α . volume_tac) :
  Prop :=
indep_setsₖ {s | measurable_set[m₁] s} {s | measurable_set[m₂] s} ν μ

/-- A family of sets is independent if the family of measurable space structures they generate is
independent. For a set `s`, the generated measurable space has measurable sets `∅, s, sᶜ, univ`. -/
def Indep_setₖ [measurable_space Ω] (s : ι → set Ω) (ν : α → measure Ω)
  (μ : measure α . volume_tac) : Prop :=
Indepₖ (λ i, generate_from {s i}) ν μ

/-- Two sets are independent if the two measurable space structures they generate are independent.
For a set `s`, the generated measurable space structure has measurable sets `∅, s, sᶜ, univ`. -/
def indep_setₖ [measurable_space Ω] (s t : set Ω) (ν : α → measure Ω)
  (μ : measure α . volume_tac) : Prop :=
indepₖ (generate_from {s}) (generate_from {t}) ν μ
...
def Indep_funₖ [measurable_space Ω] {β : ι → Type*} (m : Π (x : ι), measurable_space (β x))
  ...
  (μ : measure α . volume_tac) : Prop :=
Indepₖ (λ x, measurable_space.comap (f x) (m x)) ν μ
...
def indep_funₖ {β γ} [measurable_space Ω] [mβ : measurable_space β] [mγ : measurable_space γ]
  ...
  (μ : measure α . volume_tac) : Prop :=
indepₖ (measurable_space.comap f mβ) (measurable_space.comap g mγ) ν μ

end definitions

section indep

variables {mα : measurable_space α} {m₁ m₂ m₃ m : measurable_space Ω} {ν : α → measure Ω}
  {μ : measure α} {s₁ s₂ s₃ : set (set Ω)}

@[symm] lemma indep_setsₖ.symm (h : indep_setsₖ s₁ s₂ ν μ) :
  indep_setsₖ s₂ s₁ ν μ :=
begin
...
@[symm] lemma indepₖ.symm (h : indepₖ m₁ m₂ ν μ) :
  indepₖ m₂ m₁ ν μ :=
indep_setsₖ.symm h

lemma indepₖ_bot_right (m' : measurable_space Ω) {mα : measurable_space α} {m : measurable_space Ω}
  {ν : α → measure Ω} {μ : measure α} [∀ a, is_probability_measure (ν a)] :
  indepₖ m' ⊥ ν μ :=
begin
  intros s t hs ht,
...
lemma indep_setₖ_empty_right [∀ a, is_probability_measure (ν a)] (s : set Ω) :
  indep_setₖ s ∅ ν μ :=
by { simp only [indep_setₖ, generate_from_singleton_empty], exact indepₖ_bot_right _, }

lemma indep_setₖ_empty_left [∀ a, is_probability_measure (ν a)] (s : set Ω) :
  indep_setₖ ∅ s ν μ :=
(indep_setₖ_empty_right s).symm

lemma indep_setsₖ_of_indep_setsₖ_of_le_left (h_indep : indep_setsₖ s₁ s₂ ν μ) (h31 : s₃ ⊆ s₁) :
  indep_setsₖ s₃ s₂ ν μ :=
λ t1 t2 ht1 ht2, h_indep t1 t2 (set.mem_of_subset_of_mem h31 ht1) ht2

lemma indep_setsₖ_of_indep_setsₖ_of_le_right (h_indep : indep_setsₖ s₁ s₂ ν μ) (h32 : s₃ ⊆ s₂) :
  indep_setsₖ s₁ s₃ ν μ :=
λ t1 t2 ht1 ht2, h_indep t1 t2 ht1 (set.mem_of_subset_of_mem h32 ht2)

lemma indepₖ_of_indepₖ_of_le_left (h_indep : indepₖ m₁ m₂ ν μ) (h31 : m₃ ≤ m₁) :
  indepₖ m₃ m₂ ν μ :=
λ t1 t2 ht1 ht2, h_indep t1 t2 (h31 _ ht1) ht2

lemma indepₖ_of_indepₖ_of_le_right (h_indep : indepₖ m₁ m₂ ν μ) (h32 : m₃ ≤ m₂) :
  indepₖ m₁ m₃ ν μ :=
λ t1 t2 ht1 ht2, h_indep t1 t2 ht1 (h32 _ ht2)

lemma indep_setsₖ.union (h₁ : indep_setsₖ s₁ s₃ ν μ) (h₂ : indep_setsₖ s₂ s₃ ν μ) :
  indep_setsₖ (s₁ ∪ s₂) s₃ ν μ :=
begin
  intros t1 t2 ht1 ht2,
...
@[simp] lemma indep_setsₖ.union_iff :
  indep_setsₖ (s₁ ∪ s₂) s₃ ν μ ↔ indep_setsₖ s₁ s₃ ν μ ∧ indep_setsₖ s₂ s₃ ν μ :=
...
lemma indep_setsₖ.Union {s : ι → set (set Ω)} (hyp : ∀ n, indep_setsₖ (s n) s₂ ν μ) :
  indep_setsₖ (⋃ n, s n) s₂ ν μ :=
begin
  intros t1 t2 ht1 ht2,
...
lemma indep_setsₖ.bUnion {s : ι → set (set Ω)} {u : set ι}
  (hyp : ∀ n ∈ u, indep_setsₖ (s n) s₂ ν μ) :
  indep_setsₖ (⋃ n ∈ u, s n) s₂ ν μ :=
begin
  intros t1 t2 ht1 ht2,
...
lemma indep_setsₖ.inter  (s₂ : set (set Ω)) (h₁ : indep_setsₖ s₁ s₃ ν μ) :
  indep_setsₖ (s₁ ∩ s₂) s₃ ν μ :=
λ t1 t2 ht1 ht2, h₁ t1 t2 ((set.mem_inter_iff _ _ _).mp ht1).left ht2

lemma indep_setsₖ.Inter {s : ι → set (set Ω)} (h : ∃ n, indep_setsₖ (s n) s₂ ν μ) :
  indep_setsₖ (⋂ n, s n) s₂ ν μ :=
by {intros t1 t2 ht1 ht2, cases h with n h, exact h t1 t2 (set.mem_Inter.mp ht1 n) ht2 }

lemma indep_setsₖ.bInter {s : ι → set (set Ω)} {u : set ι} (h : ∃ n ∈ u, indep_setsₖ (s n) s₂ ν μ) :
  indep_setsₖ (⋂ n ∈ u, s n) s₂ ν μ :=
begin
  intros t1 t2 ht1 ht2,
...
lemma indep_setsₖ_singleton_iff {s t : set Ω} :
  indep_setsₖ {s} {t} ν μ ↔ ∀ᵐ a ∂μ, ν a (s ∩ t) = ν a s * ν a t :=
⟨λ h, h s t rfl rfl,
...
variables {mα : measurable_space α} {m₀ : measurable_space Ω} {ν : α → measure Ω} {μ : measure α}

lemma Indep_setsₖ.indep_setsₖ {s : ι → set (set Ω)} (h_indep : Indep_setsₖ s ν μ)
  {i j : ι} (hij : i ≠ j) :
  indep_setsₖ (s i) (s j) ν μ :=
begin
  classical,
  intros t₁ t₂ ht₁ ht₂,
...
lemma Indepₖ.indepₖ {m : ι → measurable_space Ω} (h_indep : Indepₖ m ν μ) {i j : ι} (hij : i ≠ j) :
  indepₖ (m i) (m j) ν μ :=
begin
  change indep_setsₖ ((λ x, measurable_set[m x]) i) ((λ x, measurable_set[m x]) j) ν μ,
  exact Indep_setsₖ.indep_setsₖ h_indep hij,
end

lemma Indep_funₖ.indep_funₖ  {β : ι → Type*}
  {m : Π x, measurable_space (β x)} {f : Π i, Ω → β i} (hf_Indep : Indep_funₖ m f ν μ)
  {i j : ι} (hij : i ≠ j) :
  indep_funₖ (f i) (f j) ν μ :=
hf_Indep.indepₖ hij
...
lemma indepₖ.indep_setsₖ {s1 s2 : set (set Ω)}
  (h_indep : indepₖ (generate_from s1) (generate_from s2) ν μ) :
  indep_setsₖ s1 s2 ν μ :=
...
variables {mα : measurable_space α} {m₁ m₂ m₀ : measurable_space Ω} {ν : α → measure Ω}
  {μ : measure α}

private lemma indep_setsₖ.indepₖ_aux [∀ a, is_probability_measure (ν a)] {p1 p2 : set (set Ω)}
  (h2 : m₂ ≤ m₀) (hp2 : is_pi_system p2) (hpm2 : m₂ = generate_from p2)
  (hyp : indep_setsₖ p1 p2 ν μ) {t1 t2 : set Ω} (ht1 : t1 ∈ p1) (ht1m : measurable_set[m₀] t1)
...
lemma indep_setsₖ.indepₖ [∀ a, is_probability_measure (ν a)] {p1 p2 : set (set Ω)}
  (h1 : m₁ ≤ m₀) (h2 : m₂ ≤ m₀)
  (hp1 : is_pi_system p1) (hp2 : is_pi_system p2) (hpm1 : m₁ = generate_from p1)
  (hpm2 : m₂ = generate_from p2) (hyp : indep_setsₖ p1 p2 ν μ) :
  indepₖ m₁ m₂ ν μ :=
begin
  intros t1 t2 ht1 ht2,
...
lemma indep_setsₖ.indepₖ' [∀ a, is_probability_measure (ν a)] {p1 p2 : set (set Ω)}
  ...
  (hp1 : is_pi_system p1) (hp2 : is_pi_system p2) (hyp : indep_setsₖ p1 p2 ν μ) :
  indepₖ (generate_from p1) (generate_from p2) ν μ :=
hyp.indepₖ (generate_from_le hp1m) (generate_from_le hp2m) hp1 hp2 rfl rfl

lemma indep_setsₖ_pi_Union_Inter_of_disjoint [∀ a, is_probability_measure (ν a)]
  {s : ι → set (set Ω)} {S T : set ι}
  (h_indep : Indep_setsₖ s ν μ) (hST : disjoint S T) :
  indep_setsₖ (pi_Union_Inter s S) (pi_Union_Inter s T) ν μ :=
begin
...
lemma Indep_setₖ.indepₖ_generate_from_of_disjoint [∀ a, is_probability_measure (ν a)] {s : ι → set Ω}
  (hsm : ∀ n, measurable_set (s n)) (hs : Indep_setₖ s ν μ) (S T : set ι) (hST : disjoint S T) :
  indepₖ (generate_from {t | ∃ n ∈ S, s n = t}) (generate_from {t | ∃ k ∈ T, s k = t}) ν μ :=
begin
...
lemma indepₖ_supr_of_disjoint [∀ a, is_probability_measure (ν a)] {m : ι → measurable_space Ω}
  (h_le : ∀ i, m i ≤ m₀) (h_indep : Indepₖ m ν μ) {S T : set ι} (hST : disjoint S T) :
  indepₖ (⨆ i ∈ S, m i) (⨆ i ∈ T, m i) ν μ :=
begin
...
lemma indepₖ_supr_of_directed_le {m : ι → measurable_space Ω}
  ...
  (h_indep : ∀ i, indepₖ (m i) m₂ ν μ) (h_le : ∀ i, m i ≤ m₀) (h_le' : m₂ ≤ m₀)
  (hm : directed (≤) m) :
  indepₖ (⨆ i, m i) m₂ ν μ :=
begin
  ...
  have h_pi_system_indep : indep_setsₖ (⋃ n, p n) p' ν μ,
...
lemma Indep_setₖ.indepₖ_generate_from_lt [preorder ι] [∀ a, is_probability_measure (ν a)]
  {s : ι → set Ω} (hsm : ∀ n, measurable_set (s n)) (hs : Indep_setₖ s ν μ) (i : ι) :
  indepₖ (generate_from {s i}) (generate_from {t | ∃ j < i, s j = t}) ν μ :=
begin
...
lemma Indep_setₖ.indepₖ_generate_from_le [linear_order ι] [∀ a, is_probability_measure (ν a)]
  {s : ι → set Ω} (hsm : ∀ n, measurable_set (s n)) (hs : Indep_setₖ s ν μ)
  (i : ι) {k : ι} (hk : i < k) :
  indepₖ (generate_from {s k}) (generate_from {t | ∃ j ≤ i, s j = t}) ν μ :=
begin
...
lemma Indep_setₖ.indepₖ_generate_from_le_nat [∀ a, is_probability_measure (ν a)]
  {s : ℕ → set Ω} (hsm : ∀ n, measurable_set (s n)) (hs : Indep_setₖ s ν μ) (n : ℕ):
  indepₖ (generate_from {s (n + 1)}) (generate_from {t | ∃ k ≤ n, s k = t}) ν μ :=
hs.indepₖ_generate_from_le hsm _ n.lt_succ_self

lemma indepₖ_supr_of_monotone [semilattice_sup ι] {m : ι → measurable_space Ω}
  [∀ a, is_probability_measure (ν a)]
  (h_indep : ∀ i, indepₖ (m i) m₂ ν μ) (h_le : ∀ i, m i ≤ m₀) (h_le' : m₂ ≤ m₀) (hm : monotone m) :
  indepₖ (⨆ i, m i) m₂ ν μ :=
...
lemma indepₖ_supr_of_antitone [semilattice_inf ι] {m : ι → measurable_space Ω}
  [∀ a, is_probability_measure (ν a)]
  (h_indep : ∀ i, indepₖ (m i) m₂ ν μ) (h_le : ∀ i, m i ≤ m₀) (h_le' : m₂ ≤ m₀) (hm : antitone m) :
  indepₖ (⨆ i, m i) m₂ ν μ :=
...
lemma Indep_setsₖ.pi_Union_Inter_of_not_mem {π : ι → set (set Ω)} {a : ι} {S : finset ι}
  (hp_ind : Indep_setsₖ π ν μ) (haS : a ∉ S) :
  indep_setsₖ (pi_Union_Inter π S) (π a) ν μ :=
begin
...
theorem Indep_setsₖ.Indepₖ [∀ a, is_probability_measure (ν a)] (m : ι → measurable_space Ω)
  (h_le : ∀ i, m i ≤ m₀) (π : ι → set (set Ω)) (h_pi : ∀ n, is_pi_system (π n))
  (h_generate : ∀ i, m i = generate_from (π i)) (h_ind : Indep_setsₖ π ν μ) :
  Indepₖ m ν μ :=
begin
  classical,
  ...
  have h_indep : indepₖ m_p (m a) ν μ,
...
lemma indep_setₖ_iff_indep_setsₖ_singleton (hs_meas : measurable_set s) (ht_meas : measurable_set t)
  [∀ a, is_probability_measure (ν a)] :
  indep_setₖ s t ν μ ↔ indep_setsₖ {s} {t} ν μ :=
...
lemma indep_setₖ_iff_measure_inter_eq_mul (hs_meas : measurable_set s) (ht_meas : measurable_set t)
  [∀ a, is_probability_measure (ν a)] :
  indep_setₖ s t ν μ ↔ ∀ᵐ a ∂μ, ν a (s ∩ t) = ν a s * ν a t :=
...
lemma indep_setsₖ.indep_setₖ_of_mem (hs : s ∈ S) (ht : t ∈ T)
  ...
  [∀ a, is_probability_measure (ν a)]
  (h_indep : indep_setsₖ S T ν μ) :
  indep_setₖ s t ν μ :=
(indep_setₖ_iff_measure_inter_eq_mul hs_meas ht_meas).mpr (h_indep s t hs ht)

lemma indepₖ.indep_setₖ_of_measurable_set (h_indep : indepₖ m₁ m₂ ν μ) {s t : set Ω}
...
lemma indepₖ_iff_forall_indep_setₖ (m₁ m₂ : measurable_space Ω) {m0 : measurable_space Ω}
  {mα : measurable_space α} {ν : α → measure Ω} {μ : measure α} :
  indepₖ m₁ m₂ ν μ ↔ ∀ s t, measurable_set[m₁] s → measurable_set[m₂] t → indep_setₖ s t ν μ :=
...
variables {β β' γ γ' : Type*} {mα : measurable_space α} {mΩ : measurable_space Ω}
  {ν : α → measure Ω} {μ : measure α} {f : Ω → β} {g : Ω → β'}
...
lemma indep_funₖ_iff_indep_setₖ_preimage {mβ : measurable_space β} {mβ' : measurable_space β'}
  ...
  indep_funₖ f g ν μ ↔
    ∀ s t, measurable_set s → measurable_set t → indep_setₖ (f ⁻¹' s) (g ⁻¹' t) ν μ :=
begin
...
@[symm] lemma indep_funₖ.symm {mβ : measurable_space β} {f g : Ω → β} (hfg : indep_funₖ f g ν μ) :
  indep_funₖ g f ν μ :=
hfg.symm

lemma indep_funₖ.ae_eq {mβ : measurable_space β} {f g f' g' : Ω → β}
  (hfg : indep_funₖ f g ν μ) (hf : ∀ᵐ a ∂μ, f =ᵐ[ν a] f') (hg : ∀ᵐ a ∂μ, g =ᵐ[ν a] g') :
  indep_funₖ f' g' ν μ :=
begin
...
lemma indep_funₖ.comp {mβ : measurable_space β} {mβ' : measurable_space β'}
  ...
  (hfg : indep_funₖ f g ν μ) (hφ : measurable φ) (hψ : measurable ψ) :
  indep_funₖ (φ ∘ f) (ψ ∘ g) ν μ :=
begin
  rintro _ _ ⟨A, hA, rfl⟩ ⟨B, hB, rfl⟩,
  apply hfg,
...
lemma Indep_funₖ.indep_funₖ_finset [∀ a, is_probability_measure (ν a)]
  {ι : Type*} {β : ι → Type*} {m : Π i, measurable_space (β i)}
  {f : Π i, Ω → β i} (S T : finset ι) (hST : disjoint S T) (hf_Indep : Indep_funₖ m f ν μ)
  (hf_meas : ∀ i, measurable (f i)) :
  indep_funₖ (λ a (i : S), f i a) (λ a (i : T), f i a) ν μ :=
begin
...
lemma Indep_funₖ.indep_funₖ_prod [∀ a, is_probability_measure (ν a)]
  {ι : Type*} {β : ι → Type*} {m : Π i, measurable_space (β i)}
  {f : Π i, Ω → β i} (hf_Indep : Indep_funₖ m f ν μ) (hf_meas : ∀ i, measurable (f i))
  (i j k : ι) (hik : i ≠ k) (hjk : j ≠ k) :
  indep_funₖ (λ a, (f i a, f j a)) (f k) ν μ :=
begin
  classical,
...
@[to_additive]
lemma Indep_funₖ.mul [∀ a, is_probability_measure (ν a)]
  {ι : Type*} {β : Type*} {m : measurable_space β} [has_mul β] [has_measurable_mul₂ β]
  {f : ι → Ω → β} (hf_Indep : Indep_funₖ (λ _, m) f ν μ) (hf_meas : ∀ i, measurable (f i))
  (i j k : ι) (hik : i ≠ k) (hjk : j ≠ k) :
  indep_funₖ (f i * f j) (f k) ν μ :=
begin
  have : indep_funₖ (λ ω, (f i ω, f j ω)) (f k) ν μ := hf_Indep.indep_funₖ_prod hf_meas i j k hik hjk,
  change indep_funₖ ((λ p : β × β, p.fst * p.snd) ∘ (λ ω, (f i ω, f j ω))) (id ∘ (f k)) ν μ,
  exact indep_funₖ.comp this (measurable_fst.mul measurable_snd) measurable_id,
end

@[to_additive]
lemma Indep_funₖ.indep_funₖ_finset_prod_of_not_mem [∀ a, is_probability_measure (ν a)]
  {ι : Type*} {β : Type*} {m : measurable_space β} [comm_monoid β] [has_measurable_mul₂ β]
  {f : ι → Ω → β} (hf_Indep : Indep_funₖ (λ _, m) f ν μ) (hf_meas : ∀ i, measurable (f i))
  {s : finset ι} {i : ι} (hi : i ∉ s) :
  indep_funₖ (∏ j in s, f j) (f i) ν μ :=
begin
  classical,
...
@[to_additive]
lemma Indep_funₖ.indep_funₖ_prod_range_succ [∀ a, is_probability_measure (ν a)]
  {β : Type*} {m : measurable_space β} [comm_monoid β] [has_measurable_mul₂ β]
  {f : ℕ → Ω → β} (hf_Indep : Indep_funₖ (λ _, m) f ν μ) (hf_meas : ∀ i, measurable (f i))
  (n : ℕ) :
  indep_funₖ (∏ j in finset.range n, f j) (f n) ν μ :=
...
lemma Indep_setₖ.Indep_funₖ_indicator [has_zero β] [has_one β] {m : measurable_space β}
  {s : ι → set Ω} (hs : Indep_setₖ s ν μ) :
  Indep_funₖ (λ n, m) (λ n, (s n).indicator (λ ω, 1)) ν μ :=
begin
  classical,
