...
lemma countable_coe_iff {s : set α} : countable s ↔ s.countable := iff.rfl
...
⟨λ ⟨f, hf⟩, by { lift f to α → ℕ using trivial, exact ⟨f, inj_on_iff_injective.2 hf⟩},
...
let ⟨h⟩ := hs in @finite.to_countable _ h.finite
